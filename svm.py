...
def svm(x_train, x_val, y_train, y_val, kern):

    clf = make_pipeline(StandardScaler(), SVC(kernel = kern, gamma='auto', verbose = True))
...
for kern in kernel:
    hyperparam[kern] = {}
    acc, auc = svm(x_train, x_val, y_train, y_val, kern)
